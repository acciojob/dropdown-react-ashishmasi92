...
function getCitiesOfStates(state) {
	...
	return currentState.city.map((val) => {
		return val.name
	})
}
...
function getLandMarks(state, city) {
	let currentState = states.find((val) => {
...
if(!currentState){
	...
	return currnetCity.landmarks.map((val) => {
		return val.name
	})
}


// console.log(getLandMarks("Jharkhand", "Wasseypur"));
...
function App() {
	...
	let [selectState, setSelectState] = useState("Madhya Pradesh")
	let [city, setCity] = useState([])
	let [selectedCity, setSelectedCity] = useState("Indore")
	let [landmarks, setLandMarks] = useState([])
	let [selectLandMark, setLandMark] = useState("Mhow")
	...
	useEffect(() => {
		...
		let c = getCitiesOfStates(selectState)
		setCity(c)
	}, [selectState])


	useEffect(() => {
		let x = getCitiesOfStates(selectState)
		setSelectedCity(x[0])
		setLandMarks(getLandMarks(selectState, x[0]))
	}, [selectState])

	useEffect(() => {
		let x = getLandMarks(selectState, selectedCity)
		setLandMarks(x)
		setLandMark(x[0])
	}, [selectedCity,selectState])
